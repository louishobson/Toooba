...
bsc -bdir $BUILD_DIR -simdir $BUILD_DIR ./../../src_Core/RISCY_OOO/coherence/src/RWBramCoreSequential.bsv && \
...
bsc -bdir $BUILD_DIR -show-range-conflict -simdir $BUILD_DIR -sim -g mkRWBramCoreSequentialTest ./../../src_Testbench/Unit/CheriPrefetcher_test.bsv && \
bsc -bdir $BUILD_DIR -simdir $BUILD_DIR -sim -e mkRWBramCoreSequentialTest -o bramseq_test && \ 
./bramseq_test  && \
rm ./bramseq_test*
